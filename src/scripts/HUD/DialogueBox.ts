...
export class WELLY_DialogueBox extends TextBox
{
    constructor(scene: Welly_Scene, config: TextBox.IConfig)
    {
        ...
        config.layoutMode = 1;
        const background = scene.rexUI.add.roundRectangle(0, 0, config.width - (config.space.left as number) - (config.space.right as number), config.height - (config.space.top as number) - (config.space.bottom as number), 8, WELLY_Utils.hexColorToNumber(CST.STYLE.COLOR.WHITE));
