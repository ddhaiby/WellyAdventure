...
export class WELLY_BottomMenu extends Phaser.GameObjects.Container
{
    ...
    constructor(scene: WELLY_BaseScene, x: number, y: number, width: number, height: number)
    {
        ...
        const background = this.scene.add.rectangle(0, 0, this.width, this.height, WELLY_Utils.hexColorToNumber(WELLY_CST.STYLE.COLOR.WHITE), 1);
        this.add(background);
    ...
    protected initTurretButtonList(): void
    {
        ...
        for (const turretData of this.turretsData)
        {
            ...
            const turretButton = this.scene.rexUI.add.label({
                background: this.scene.rexUI.add.roundRectangle(0, 0, turretContainer.width, turretContainer.height, 10, WELLY_Utils.hexColorToNumber(WELLY_CST.STYLE.COLOR.LIGHT_BLUE), WELLY_Utils.hexColorToNumber(WELLY_CST.STYLE.COLOR.BLUE)),
            ...
            const turretCounterText = this.scene.add.text(turretContainer.width * 0.5 - 4, turretContainer.height * 0.5 - 4, `x${turretData.maxInstances}`, { fontSize: "16px", color: WELLY_CST.STYLE.COLOR.BLUE, fontFamily: WELLY_CST.STYLE.TEXT.FONT_FAMILY });
            ...
            const priceText = this.scene.add.text(0, 0, turretData.gameStatsPerLevel[0].price.toString(), { fontSize: "16px", color: WELLY_CST.STYLE.COLOR.BLUE, fontFamily: WELLY_CST.STYLE.TEXT.FONT_FAMILY });
